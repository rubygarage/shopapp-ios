...
class MagentoAPICustomerSpec: MagentoAPIBaseSpec {
    override func spec() {
        ...
        let addAddressResponseObjects: [Any] = [customerWithAddressJson, countriesJson, customerWithAddressesJson]
        let updateAddressDataErrorResponseObjects: [Any] = [customerWithAddressJson, countriesJson, countriesJson]
        let updateAddressResponseObjects: [Any] = [customerWithAddressJson, countriesJson, customerWithAddressJson]
            ...
            context("if response has error") {
                context("because of request data") {
                    it("needs to return error") {
                        ...
                        waitUntil { done in
                            self.api.signUp(firstName: "First", lastName: "Last", email: "user@mail.com", password: "password", phone: "+11011231231") { (response, error) in
        ...
        describe("when user logs in") {
            context("if response has error") {
                it("needs to return error") {
                    ...
                    waitUntil { done in
                        self.api.signIn(email: "user@mail.com", password: "password") { (response, error) in
        ...
        describe("when user logs out") {
            it("needs to return success") {
                waitUntil { done in
                    self.api.signOut { (response, error) in
                        self.checkSuccessResponse(response, error: error)
        ...
        describe("when user checks logged in status") {
            context("and it's false") {
                it("needs to return fail") {
                    ...
                    waitUntil { done in
                        self.api.isSignedIn() { (isSignedIn, _) in
        ...
        describe("when user resets password") {
            context("if response has error") {
                it("needs to return error") {
                    ...
                    waitUntil { done in
                        self.api.resetPassword(email: "user@mail.com") { (response, error) in
                            self.checkUnsuccessResponse(response, error: error, errorMessage: errorMessage)
            ...
            context("if response has error") {
                context("because of session or request data") {
                    it("needs to return error") {
                        ...
                        waitUntil { done in
                            self.api.updateCustomer(firstName: "First", lastName: "Last", phone: "+11011231231") { (response, error) in
                                self.checkUnsuccessResponse(response, error: error)
            ...
            context("if response has error") {
                context("because of session data") {
                    it("needs to return error") {
                        ...
                        waitUntil { done in
                            self.api.updatePassword(password: "password") { (response, error) in
                                self.checkUnsuccessResponse(response, error: error)
            ...
            context("if response has error") {
                context("because of request data") {
                    it("needs to return error") {
                        ...
                        waitUntil { done in
                            self.api.addCustomerAddress(address: address) { (response, error) in
                                self.checkUnsuccessResponse(response, error: error)
            ...
            context("if response has error") {
                context("because of request data") {
                    it("needs to return error") {
                        ...
                        waitUntil { done in
                            self.api.updateCustomerAddress(address: address) { (response, error) in
                                self.checkUnsuccessResponse(response, error: error)
            ...
            context("if response has error") {
                context("because of request data") {
                    it("needs to return error") {
                        ...
                        waitUntil { done in
                            self.api.setDefaultShippingAddress(addressId: "id") { (response, error) in
                                self.checkUnsuccessResponse(response, error: error)
                ...
                context("because of api") {
                    it("needs to return error") {
                        for index in 0..<addressResponseObjects.count {
                            ...
                            waitUntil { done in
                                self.api.setDefaultShippingAddress(addressId: customerLastAddressId) { (response, error) in
                                    self.checkUnsuccessResponse(response, error: error)
            ...
            context("if response has error") {
                context("because of request data") {
                    it("needs to return error") {
                        ...
                        waitUntil { done in
                            self.api.deleteCustomerAddress(addressId: "id") { (response, error) in
                                self.checkUnsuccessResponse(response, error: error)
                ...
                context("because of api") {
                    it("needs to return error") {
                        for index in 0..<deleteAddressApiErrorResponseObjects.count {
                            ...
                            waitUntil { done in
                                self.api.deleteCustomerAddress(addressId: customerLastAddressId) { (response, error) in
                                    self.checkUnsuccessResponse(response, error: error)
